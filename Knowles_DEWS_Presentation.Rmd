...
- Provides validated predictions to start and end the school year based on data 
availability
- Built on open source packages for the R statistical computing language
- Coupled with a user guide, documentation, and help desk support
...
- Choosing how to measure accuracy
...
validity in some school settings *(Heppen and Therriault, 2008; Easton and Allensworth, 2005 & 2007; Allensworth, 2013, Balfanz and Herzog, 2006; Balfanz and Iver, 2007)*.
- The downside is that they overemphasize individual at the risk of oversimplifying 
the mechanisms underlying risk *(Gleason and Dynarski, 2002)*. 
...
>- **Early** identification in time to make interventions (grades 5-8)
...
We can use a confusion matrix to explore binary classification choices
...
all non-graduates (recall), and the fraction of false non-graduates out of all graduates 
(precisions)
...
substantial variation between districts), and a vast majority of school districts 
with a single high school
- Wisconsin does not have high enough coursework data quality to support many 
EWS analyses
- At state level we have lots of observations, but relatively few measures
...
- A checklist model will not work statewide becuse of delays in data availability 
...
- Look at setting up a machine learning workflow
...
- Start with some more basic models, build complexity in order to find increased 
...
- Blend a localized prediction with state model prediction for each district/school
